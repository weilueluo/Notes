...
Name of specific thing. Such as `cat` and `building`, tagged NN. When we have the plural form, `cats` and `buildings`, it is tagged NNS.
...
When we refer to an unique entity such as `John` and `Mary`, we have a proper noun, tagged NNP. When we have the plural form of them, such as `Johns` and `Marys`, it is tagged NNPS.
...
This category includes personal pronoun proper such as `I`, `he` and `they` and reflesive pronoun ending `-self` or `-selves` such as `themselves` and `herself`.
...
These are words that does not end with `-s` in their singlar form, such as `could`, `shall` and `will`.

### [Adverb](https://en.wikipedia.org/wiki/Adverb) RB (副词)

Words that modify verb or verb phrase, this includes most words end with `-ly` as well as words like `quite`, `too` and `very`; posthead marker like `enough` in `good enough` and `indeed` in `very well indeed`; negative markers like `not` and `never`.
...
Basically comparative & superlative adjective that used to modify verb or verb phrases instead of noun and noun phrases.
...
When determiner precedes another determiner or  possessive pronoun it is called predeterminer. This is because any noun phrase can only has one determiner. For example: `such` in `such a good time` is tagged as predeterminer, but note that `such` in `the only such case` is tagged as JJ.
...
It is an unstressed `there` which trigger invasion of the inflacted verb and logical subject of the sentence. For example, `there was a party in progress`.

### Foreign Word FW

It is the foreign words, just note that `e.g., etc., i.e.` are abbreviations of foreign terms so they are tagged FW.
...
- JJR
  - This is a **faster** car than the previous one.
  - He has a **higher** degree than me.
...
|         | **Determiner**            | **Pronoun**               |
| ------- | ------------------------- | ------------------------- |
| *what*  | What colour is his house? | What did she buy for him? |
